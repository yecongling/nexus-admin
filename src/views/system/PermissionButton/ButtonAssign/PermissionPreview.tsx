...
import { Card, Descriptions, Tag, Table, Space, Spin, Tabs, Badge } from 'antd';
 ...
 * 展示角色的完整权限信息
 */
const PermissionPreview: React.FC<PermissionPreviewProps> = ({ roleId }) => {
  /**
   * 查询角色基本信息
   */
  const { data: roleInfo, isLoading: roleInfoLoading } = useQuery({
    queryKey: ['role-info', roleId],
    queryFn: () => roleService.getRoleDetail(roleId),
    enabled: !!roleId,
  });
      ...
      <Card title="权限统计" size="small">
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">{rolePermissions?.menuPermissions?.length || 0}</div>
            <div className="text-sm text-gray-500">菜单权限</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-orange-600">{rolePermissions?.buttonPermissions?.length || 0}</div>
